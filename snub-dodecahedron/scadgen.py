...
def generate_scad_single(piece, params: FoldParams, name="piece") -> str:
    """A standalone .scad for one piece, centered on the origin (bed center)."""
    x0, y0, x1, y1 = piece.bbox()
    cx = (x0 + x1) / 2 * params.edge_mm
    cy = (y0 + y1) / 2 * params.edge_mm
    body = _scad_for_piece(piece, params, (-cx, -cy), name)
    return "\n".join(_header(params)) + body + f"\n\n{name}();\n"
