...
def inspect_repo(
    ...
    try:
        ...
        return RepoScanResult(repo=repo, tags=[], reports=[])

    for tag in tags:
        report = inspect_tag(client, org, repo, tag)
        if report is not None:
            reports.append(report)
    return RepoScanResult(repo=repo, tags=tags, reports=reports)
...
def write_markdown(
    ...
    hardened.sort(key=lambda item: (-item.findings, item.repo, item.tag))

    repos_with_hardening = {report.repo for report in hardened}
    unhardened = sorted(
        (result for result in scan_results if result.repo not in repos_with_hardening),
        key=lambda item: item.repo,
    )

    lines = [
        f"# {org} Hardening Summary",
        "",
        ...
        f"- Repos with hardening applied: {summary['repos_with_hardening_applied']}",
        f"- Tags with hardening applied: {summary['tags_with_hardening_applied']}",
        f"- Repos with no hardened tags: {len(unhardened)}",
        "",
    ...
    for report in hardened:
        finding_types = ", ".join(report.finding_types) if report.finding_types else "-"
        lines.append(
            f"| {report.repo} | {report.tag} | {report.findings} | {report.iterations} "
            f"| {finding_types} | [HARDENING.md]({report.url}) |"
        )
    ...
    for result in unhardened:
        all_tags = ", ".join(result.tags) if result.tags else "-"
        report_tags = ", ".join(report.tag for report in result.reports) if result.reports else "-"
        lines.append(f"| {result.repo} | {all_tags} | {report_tags} |")
...
def main() -> int:
    ...
    parser.add_argument("--output-markdown", help="Write markdown summary with hardened and unhardened tables")
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as pool:
        futures = {
            pool.submit(inspect_repo, client, args.org, repo["name"]): repo["name"]
            for repo in repos
        }
        completed = 0
        for future in as_completed(futures):
            ...
            try:
                ...
                for report in result.reports:
                    if not hardened_only or report.findings > 0:
    ...
    scan_results.sort(key=lambda item: item.repo)
    ...
    if args.output_markdown:
        write_markdown(args.output_markdown, args.org, scan_results, summary)
