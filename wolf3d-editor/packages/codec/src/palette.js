// The Wolfenstein 3D game palette (256 entries, 6-bit VGA RGB components).
// Values transcribed from the engine's gamepal (wolfpal.inc in Wolf4SDL,
// originally GAMEPAL.OBJ in WOLFSRC). Index 255 is the sprite-transparency
// key ('sexy pink').

/** 256*3 six-bit VGA components in palette order. */
export const WOLF_PALETTE_6BIT = new Uint8Array([
...
/** 256*3 eight-bit RGB (VGA 6-bit scaled, replicating top bits). */
export const WOLF_PALETTE = (() => {
  const out = new Uint8Array(768);
  for (let i = 0; i < 768; i++) {
    const v = WOLF_PALETTE_6BIT[i];
    out[i] = (v << 2) | (v >> 4);
  }
  return out;
})();
...
/**
 * Find the nearest palette index for an RGB color (excludes index 255, the
 * transparency key, so imported art never becomes accidentally transparent).
...
export function nearestColor(r, g, b) {
  ...
  for (let i = 0; i < 255; i++) {
    const dr = WOLF_PALETTE[i * 3] - r;
    const dg = WOLF_PALETTE[i * 3 + 1] - g;
    const db = WOLF_PALETTE[i * 3 + 2] - b;
